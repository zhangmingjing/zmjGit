...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        plusBtn.layer.shadowOffset=CGSizeMake(0, 3);
...
-(void)layoutSubviews{
    ...
    for (UIView *view in self.subviews) {
        if ([view isEqual:self.centerTitle]) {//self.centerButton
            ...
            [control addTarget:self action:@selector(didSelectControl:) forControlEvents:UIControlEventTouchUpInside];
        }
    }
}

-(void)didSelectControl:(UIControl *)control{
    if ([self.tabDelegate respondsToSelector:@selector(tabBar:clickSystemButton:)]) {
        [self.tabDelegate tabBar:self clickSystemButton:control];
    }
}
