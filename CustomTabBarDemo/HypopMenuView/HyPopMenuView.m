...
- (void)initUIsize
{
    ...
    //12 是按钮距离bottomView顶部的距离
    disappearButton.center = CGPointMake(bottomView.center.x, bottomView.frame.origin.y+12+CANCELw/2.0);
}
...
- (CGRect)getFrameAtIndex:(NSUInteger)index;
{
    ...
    NSLog(@"toRows:%lu,toHeight:%f",(unsigned long)toRows,toHeight);
    CGFloat buttonViewX = (colnumIndex + 1) * margin + colnumIndex * buttonViewWidth;
    //控制按钮的Y坐标（不是3的倍数时toHeight = 230  是3的倍数时toHeight = 345）
    CGFloat buttonViewY = ((rowIndex + 1) * margin + rowIndex * buttonViewHeight) + (kH - toHeight) - 150-50;
